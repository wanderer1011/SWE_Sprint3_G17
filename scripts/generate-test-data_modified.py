...
Usage:
    ...
    python generate-test-data.py --mode kafka --kafka-route cold-only --count 5000
    python generate-test-data.py --mode mixed --count 2000

Kafka routing behavior:
    - Default (`--kafka-route pipeline`): ERROR/CRITICAL/security-like events go to telemetry-hot,
      all others go to telemetry-cold.
    - Legacy (`--kafka-route cold-only`): all events go to telemetry-cold.
"""
...
TOPIC_HOT = os.getenv("TOPIC_HOT", "telemetry-hot")
TOPIC_COLD = os.getenv("TOPIC_COLD", "telemetry-cold")

HOT_SEVERITIES = {"ERROR", "CRITICAL", "FATAL"}
SECURITY_KEYWORDS = (
    "unauthorized",
    "forbidden",
    "authentication fail",
...
def route_topic_for_kafka(event, route_mode="pipeline"):
    """Route direct-Kafka events so behavior matches the pipeline expectation."""
    if route_mode == "cold-only":
        return TOPIC_COLD

    severity = str(event.get("severity_text") or event.get("severity") or "INFO").upper()
    body = str(event.get("body") or "").lower()
    is_security = any(keyword in body for keyword in SECURITY_KEYWORDS)

    if severity in HOT_SEVERITIES or is_security:
        return TOPIC_HOT
    return TOPIC_COLD
...
def send_kafka(events, route_mode="pipeline"):
    ...
    for i, event in enumerate(events):
        topic = route_topic_for_kafka(event, route_mode=route_mode)
        ...
        if (i + 1) % 500 == 0:
            print(
                f"  Produced {i + 1}/{len(events)} events "
                f"(hot: {hot_count}, cold: {cold_count})"
            )

    producer.flush()
    producer.close()
    print(f"  Kafka route summary: hot={hot_count}, cold={cold_count}")

def send_mixed(events, route_mode="pipeline"):
    ...
    for i, event in enumerate(events):
        ...
        if (i + 1) % 200 == 0:
            print(
                ...
                f"(HTTP: {http_count}, Kafka hot: {kafka_hot_count}, Kafka cold: {kafka_cold_count})"
            )
    ...
    print(
        "  Mixed route summary: "
        f"http={http_count}, kafka_hot={kafka_hot_count}, kafka_cold={kafka_cold_count}"
    )
...
def main():
    ...
    parser.add_argument(
        "--kafka-route",
        choices=["pipeline", "cold-only"],
        default="pipeline",
        help=(
            "Kafka/mixed route strategy: 'pipeline' sends ERROR/CRITICAL/security events to telemetry-hot "
            "and others to telemetry-cold; 'cold-only' sends everything to telemetry-cold."
        ),
    )
    ...
    if args.mode == "http":
        ...
        send_kafka(events, route_mode=args.kafka_route)
    else:
        send_mixed(events, route_mode=args.kafka_route)
